      ...
      <main class="task-detail panel">
        ...
        <template v-else>
          ...
          <section v-if="task.result?.status==='completed'" class="receipt">
            ...
            <div v-if="task.result.mcp" class="mcp-receipt"><span>MCP 工具调用</span><strong>{{ task.result.mcp.server }} · {{ task.result.mcp.tool }}</strong><small>{{ task.result.mcp.transport }}</small></div>
            <el-button v-if="task.status==='completed' && task.result.action_type!=='marketing_plan'" :loading="executing" @click="rollback">回滚本次变更</el-button>
          </section>
...
const actionLabel=(value:string)=>({price_update:"调整价格",product_publish:"商品上架",product_unpublish:"商品下架",marketing_plan:"创建营销活动"} as any)[value]||"待识别"
const eventTypeLabel=(value:string)=>({task_planned:"任务规划",context_loaded:"读取数据",mcp_tool_called:"MCP 读取",policy_validated:"策略校验",change_prepared:"准备变更",campaign_prepared:"生成活动",approval_required:"审批断点",approved:"恢复执行",task_completed:"工具回执",mcp_tool_completed:"MCP 回执",task_rolled_back:"回滚完成",mcp_rollback_completed:"MCP 回滚"} as any)[value]||value
function parameterSummary(state:any){if(state.action_type==="price_update")return `价格从 ¥${state.parameters.old_price} 调整到 ¥${state.parameters.new_price}，幅度 ${state.parameters.change_pct}%`;if(state.action_type==="marketing_plan")return `创建“${state.parameters.campaign.name}”，日预算 ¥${state.parameters.campaign.daily_budget}`;return `商品状态将改为 ${state.parameters.listing_status}`}
function snapshot(value:any,type:string){return type==="price_update"?`¥${value.price} · 版本 ${value.catalog_version}`:`${value.listing_status==='listed'?'已上架':'已下架'} · 版本 ${value.catalog_version}`}
