...
| Agent | 职责 | 工具权限 |
|---|---|---|
| 主管 Agent | 接收目标、拆解任务、分派专员、检查冲突并汇总 | 不直接调用业务工具 |
| 选品专员 | 市场机会、商品表现、竞品价格和评论信号 | 商品排序、竞品分析 |
| 定价专员 | 成本、毛利、推荐售价和盈亏平衡 ACOS | 只读商品成本与价格数据 |
| Listing 专员 | Amazon 标题、五点描述、关键词和合规 | Listing 与合规工具 |
| 推广专员 | 广告结构、预算、竞价规则和效果复盘 | 广告异常、活动计划 |
| 客服专员 | FAQ、差评预警、回复原则和客户反馈 | RFM 与评价数据 |
...
3. 主管 Agent 将目标拆给选品、定价、Listing、推广和客服专员。
4. 专员在各自工具权限内生成独立结构化交付物。
5. 主管检查证据、警告和方案冲突，汇总完整经营方案。
6. 高风险动作进入人工审批，批准后由沙箱业务工具执行。
