...
def _require_approval(approved_task_id: str) -> None:
...
async def update_product_price(product_id: str, new_price: float, expected_version: int, approved_task_id: str) -> dict:
    ...
    _require_approval(approved_task_id)
    ...
    try:
        ...
        await repository.dispose()
    return {"before": before, "after": await _product_snapshot(product_id), "approved_task_id": approved_task_id}
...
async def create_marketing_campaign(product_id: str, name: str, daily_budget: float, target_acos_pct: float, approved_task_id: str) -> dict:
    ...
    return {"campaign_id": campaign_id, "product_id": product_id, "name": name, "daily_budget": daily_budget, "target_acos_pct": target_acos_pct, "status": "active", "environment": "sandbox", "approved_task_id": approved_task_id}
